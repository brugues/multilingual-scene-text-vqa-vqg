...
if not train:
    ...
    #     model_filename = 'models/pb/saved_model.pb'
    #
    ...
    graph = tf.Graph()
    trained_checkpoint_prefix = 'models/ckpt/saved_model_conv_1x1_epoch1.ckpt'
    ...
    with tf.Session(graph=graph) as sess:
        ...
        builder = tf.saved_model.builder.SavedModelBuilder('models/pb')
    ...
    txt_features = tf.placeholder(tf.float32,
                                  [batch_size, txt_feature_shape[0], txt_feature_shape[1], txt_feature_shape[2]])
    ...
    data_generator = STVQADataGenerator(gt_file_train, maxlen, image_path, img_size, fasttext_model,
                                        batch_size=batch_size)
    ...
    for epoch in range(1, n_epochs + 1):
        for it in range(data_generator.len()):
            ...
            _, l = sess.run([train_step, loss],
                            feed_dict={input_images: batch[0], txt_features: batch[1], labels: batch[3],
                                       question: batch[2],
                                       learning_rate: lr})
